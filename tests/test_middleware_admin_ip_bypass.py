...
def test_no_admin_ips_configured_behaves_normally():
    """
    When AdminIPBypassMiddleware is added with no admin IPs configured,
    it should not bypass other middlewares. Normal security applies.
    """
    app = FastAPI()
    ...
    # Add AdminIPBypassMiddleware without admin_ips configured
    app.add_middleware(
        AdminIPBypassMiddleware,
        trust_proxy_headers=True,
    )
    ...
    # Without API key, should be rejected (no bypass)
    r_missing_key = client.get("/", headers={"X-Real-IP": ADMIN_IP})
    assert r_missing_key.status_code == 401

    # With API key, rate limit should still apply
    headers = {"X-Real-IP": NON_ADMIN_IP, "X-API-Key": "valid-key"}
    assert client.get("/", headers=headers).status_code == 200
    assert client.get("/", headers=headers).status_code == 200
    r_exceed = client.get("/", headers=headers)
